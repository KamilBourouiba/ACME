...
{[}1,43{]}. Recent systems report that agents cannot epistemically
separate observation from belief {[}48{]} and that intrinsic
\emph{epistemic agency} --- including belief updating --- remains weak
in base models {[}49{]}. We present \textbf{ACME} (Adaptive Cognitive
...
inspired by cognitive architecture {[}2,3{]} and truth-maintenance
systems {[}20,21{]}; (2) hybrid graph + pgvector retrieval {[}4,5{]}
with contrarian verification {[}6,7{]}; (3) \textbf{MemoryBench v3.1}
--- fourteen sandbox-isolated scenarios with RAG {[}8{]}, MemGPT
{[}9{]}, and LangGraph-style {[}10{]} baselines scored by an LLM judge
{[}11,12{]}. On Azure OpenAI GPT-4.1, ACME achieves overall
...
SOTA on every long-context benchmark {[}35,36{]}, but the first deployed
...
reproducible production runs {[}47{]}.
...
loops. \textbf{ACME} differentiates by (i) integrated seven-engine
cognitive loop with production deployment, (ii) MemoryBench v3.1 scoring
...
Four metrics, LLM-as-judge {[}11,12{]} (semantic; keyword overlap
reported separately):
...
Lewis et al.~{[}8{]} (RAG), Packer et al.~{[}9{]} (MemGPT), and
LangGraph-style state graphs {[}10{]} --- see
...
cognitive layers compensate when the base model is weaker {[}3,34{]}.
...
tiers --- suggesting the external belief substrate {[}2,20{]} is
...
oracle split (500 Q, GPT-4.1), ACME v5 hybrid routing reaches
\textbf{87.6\%} overall vs.~\textbf{77.6\%} (RAG), with the largest
...
lifecycle management {[}20,21{]} complements --- rather than replaces
--- advances in base model capability {[}1,45{]}. Concurrent
belief-memory systems {[}48,52,53{]} validate the problem direction;
