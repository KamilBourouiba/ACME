...
retention and groundedness (\(\geq 0.90\)) but score N/A on
belief/feedback, capping their overall near 0.49. On LongMemEval, ACME
...
\textbf{Overall} = unweighted mean of all four metrics. RAG-like,
MemGPT-inspired, and LangGraph-style baselines do not maintain explicit
belief records or structured feedback traces; their Feedback and Belief
Quality scores are marked \textbf{N/A} (excluded from their overall
mean), which caps baseline overall near \textbf{0.48--0.49} even when
retention and groundedness are strong.
...
\textbf{Runs and variance.} Reported jobs are \textbf{single end-to-end
compare runs} per model tier (e.g.~job \texttt{3b31e5e3}, 725 s, zero
scenario failures). We do not yet report confidence intervals; judge
variance {[}11,12{]} remains a limitation (Section 7).
...
al.~retrieve-then-generate {[}8{]}), \textbf{MemGPT-inspired}
(paging-style context buffer per Packer et al.~{[}9{]}), and
\textbf{LangGraph-style} (accumulated session state graph per LangChain
docs {[}10{]}). See \texttt{docs/BASELINES.md}.
...
endpoint family. An additional run on a preview \texttt{gpt-5.4} Azure
deployment is reported in Appendix\textasciitilde C (not a publicly
documented OpenAI product name at submission time).
...
\textbf{N/A} rather than failed capabilities. The overall MemoryBench
...
GPT-4.1 except LangGraph-style retention 0.900).
...
0.700---cognitive layers remain stable when the base model is weaker
{[}3,34{]}.
...
Job \texttt{eb68f028} (620 s) on an Azure OpenAI preview deployment
(\texttt{gpt-5.4}, \texttt{TESTING-CHAT-BETA} resource). This model name
was available on our tenant at benchmark time but is \textbf{not} listed
in OpenAI's public GPT-4.1 documentation {[}45{]}; treat as
supplementary.

\acmeTableGptFiveFour

ACME \textbf{0.873} overall vs RAG-like \textbf{0.474} (+0.40); belief
quality \textbf{0.701}. Pattern matches GPT-4.1-mini sensitivity:
cognitive layers stable, overall gap driven by N/A baseline
belief/feedback columns.
