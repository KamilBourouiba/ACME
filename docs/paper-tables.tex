...
\newcommand{\acmeTableSetup}{%
  \acmeTableBlockCol{Experimental configuration (Azure deployment).}{%
...
\newcommand{\acmeTablePositioningCapabilities}{%
  \acmeTableBlockWide{Benchmark positioning --- evaluation capabilities (Table~1a).}{%
    \label{tab:positioning-capabilities}%
...
\newcommand{\acmeTablePositioningInfra}{%
  \acmeTableBlockWide{Benchmark positioning --- infrastructure and belief metrics (Table~1b).}{%
    ...
    \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.22\textwidth} *{5}{>{\centering\arraybackslash}p{0.13\textwidth}}@{}}
      ...
      Deployed API + persisted benchmark runs & No & No & No & No & \textbf{Yes} \\
      \bottomrule
    \end{tabular}%
  }%
}
...
\newcommand{\acmeTableMemoryBenchCI}{%
  \acmeTableBlockWide{MemoryBench v3 per-scenario bootstrap 95\% CIs (job \texttt{3b31e5e3}, $n{=}13$ scenarios). Means match Table~\ref{tab:memorybench-main}; intervals resample scenario-level judge scores.}{%
    \label{tab:memorybench-ci}%
    ...
    \begin{tabular}{@{}l *{4}{S[table-format=1.3]}@{}}
      ...
      \midrule
      \rowcolor{acmerow}
      \textbf{ACME} & \textbf{1.000} {\scriptsize (1.00--1.00)} & \textbf{1.000} {\scriptsize (1.00--1.00)} & \textbf{1.000} & \textbf{0.700} {\scriptsize (0.70--0.70)} \\
      RAG-like & 0.969 {\scriptsize (0.91--1.00)} & 0.977 {\scriptsize (0.93--1.00)} & {---} & {---} \\
      MemGPT-insp. & 0.977 {\scriptsize (0.93--1.00)} & 0.969 {\scriptsize (0.91--1.00)} & {---} & {---} \\
      LangGraph-sty. & 0.900 {\scriptsize (0.70--1.00)} & 0.977 {\scriptsize (0.93--1.00)} & {---} & {---} \\
      \bottomrule
    \end{tabular}%
    }%
  }%
...
\newcommand{\acmeTableMain}{%
  \acmeScoreTable{Primary benchmark (GPT-4.1, 13 scenarios, job \texttt{3b31e5e3}). Ret.=Retention, Gnd.=Groundedness, Fbk.=Feedback, Bel.=Belief quality, Ovr.=Overall.\label{tab:memorybench-main}}{%
      \rowcolor{acmerow}
